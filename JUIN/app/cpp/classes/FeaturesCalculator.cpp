...
// Default constructor
FeaturesCalculator::FeaturesCalculator() : _acc_norm(_threshold), _gyro_norm(_threshold), _acc_norm_nf(_threshold), _gyro_norm_nf(_threshold), _acc_x(_threshold), _acc_y(_threshold), _acc_z(_threshold), _gyro_x(_threshold), _gyro_y(_threshold), _gyro_z(_threshold) {};
...
int FeaturesCalculator::findMiddle(const vector<int>& vectorData) {
    double mid = double(vectorData.size()/2.0);
    if(fmod(mid,2.0)!=0) {
        return int(mid - .5);
    } else {
...
// Mean
double FeaturesCalculator::computeMean(const vector<double>& vectorData) {
...
// SMA
double FeaturesCalculator::computeSMA(const vector<double>& vectorData) {
    double sma = 0.0;
...
// Lerp
double FeaturesCalculator::computeLerp(double minValue, double maxValue, double interpolationPoint) {
    return minValue + interpolationPoint * (maxValue - minValue);
}

// Quantile lerp
double FeaturesCalculator::computeQuantile(const vector<double>& vectorData) {
...
// Kurtosis
double FeaturesCalculator::computeKurtosis(const vector<double>& vectorData){
    double kurtosis = 0.0;
...
// Skewness
double FeaturesCalculator::computeSkewness(const vector<double>& vectorData){
...
// Prominence
double FeaturesCalculator::computeProminence(const vector<double>& vectorData, double extrema, int first, int last) {
...
int FeaturesCalculator::getMaxArrayIndex(const valarray<complex<double>>& ArrayData) {
    struct acc_t {
        double max_value;
...
valarray<complex<double>> FeaturesCalculator::getAbsoluteArray(const valarray<complex<double>> &ArrayData) {
...
vector<double> FeaturesCalculator::computeDominantFrequency(const vector<double> &data) {
...
// Features
Features* FeaturesCalculator::computeFeatures(Sensors* sensorsData, int threshold) {
    ...
    vector<double> accx_filrered =filter->filtfilt(_acc_x);
    vector<double> accy_filrered =filter->filtfilt(_acc_y);
    vector<double> accz_filrered =filter->filtfilt(_acc_z);
    vector<double> gyrx_filrered =filter->filtfilt(_gyro_x);
    vector<double> gyry_filrered =filter->filtfilt(_gyro_y);
    vector<double> gyrz_filrered =filter->filtfilt(_gyro_z);
    ...
    if (threshold >= _threshold - 1) {
        ...
        features->set_gyro_max_frequency(computeDominantFrequency(extractVector(_gyro_norm_nf,0,_threshold))[0]);
        ...
        features->set_acc_dominant_frequency(computeDominantFrequency(extractVector(_acc_norm_nf,0,_threshold))[1]);
    }
    return features;
}
